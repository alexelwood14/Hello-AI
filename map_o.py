...
class Map:
    def __init__(self, window, file, colour, width):
        ...
        for point in range(np.shape(self.__points)[1] - 1):
            ...
            normal = np.array([[-vector.item(1)],
                               [vector.item(0)]])
            new_poly = [self.__points[:, [point]] + normal * self.width / 2,
                        self.__points[:, [point]] - normal * self.width / 2,
                        self.__points[:, [point + 1]] - normal * self.width / 2,
                        self.__points[:, [point + 1]] + normal * self.width / 2]
    ...
    def on_track(self, coords):
        ...
        for coord in coords:
            for point in range(len(self.__points)):
                ...
                mu = (line_v[0] * (coord[0] - line_s[0]) + line_v[1] * (coord[1] - line_s[1])) / (
                            line_v[0] ** 2 + line_v[1] ** 2)

                vector = coord - line_s - mu * line_v
                distance = np.linalg.norm(vector)
                if distance > self.width / 2 and 0 < mu < 1:
                    on_track = False
    ...
    def progress(self, coord):
        ...
        for point in range(len(self.__points)):
            ...
            mu = (line_v.item(0) * (coord.item(0) - line_s.item(0)) + line_v.item(1) * (
                        coord.item(1) - line_s.item(1))) / (line_v.item(0) ** 2 + line_v.item(0) ** 2)
