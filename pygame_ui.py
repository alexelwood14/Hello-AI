...
class Slider:
    ...
    def dragging(self, mouse_down):
        if mouse_down:
            ...
            if (slider_pos + self.size >= mouse_pos[0] >= slider_pos and
                    self.pos[1] + self.size >= mouse_pos[1] >= self.pos[1] - self.size) or self.grabbed:
    ...
    def render(self):
        pygame.draw.rect(self.window, self.colours[0],
                         [self.pos[0], self.pos[1] - self.size / 2, self.size * 13, self.size])
        pygame.draw.rect(self.window, self.colours[1],
                         [self.pos[0] + self.size * 14, self.pos[1] - self.size * 1.25, self.size * 4, self.size * 2.5])
        draw_text(self.window, self.name, [self.pos[0] + self.size * 9, self.pos[1] - self.size * 2.2], self.size * 2,
                  self.colours[2], "Ariel", "c")
        draw_text(self.window, "{}%".format(str(int(self.slider_pos))), [self.pos[0] + self.size * 16, self.pos[1]],
                  self.size * 2, self.colours[2], "Ariel", "c")

        pygame.draw.rect(self.window, self.colours[1],
                         [self.pos[0] + self.size * 12 * self.slider_pos / 100, self.pos[1] - self.size, self.size,
                          self.size * 2])
    ...
    def render(self):
        if self.highlighted:
            ...
            draw_text(self.window, self.text, [self.pos[0], self.pos[1]],
                      self.text_size, self.colour_1, 'calibre', 'c')
...
class Image_Button:
    ...
    def collision(self):
        ...
        if (self.pos[0] - self.size[0] / 2 <= mouse_pos[0] <= self.pos[0] + self.size[0] / 2 and
                self.pos[1] - self.size[1] / 2 <= mouse_pos[1] <= self.pos[1] + self.size[1] / 2):
            return True
        else:
            return False
    ...
    def render(self):
        ...
        draw_text(self.window, self.text, [self.pos[0], self.pos[1] + self.size[1] * 0.6], self.text_size,
                  const.COL["white"], "calibri", "c")
        if self.highlighted:
            pygame.draw.rect(self.window, const.COL["red"],
                             [self.pos[0] - self.size[0] / 2, self.pos[1] - self.size[1] / 2,
                              self.size[0], self.size[1]], 5)
        else:
            pygame.draw.rect(self.window, const.COL["grey"],
                             [self.pos[0] - self.size[0] / 2, self.pos[1] - self.size[1] / 2,
                              self.size[0], self.size[1]], 5)
...
class Up_Arrow(Arrow_Button):
    ...
    def collision(self):
        mouse_pos = pygame.mouse.get_pos()
        if (self.pos[0] - self.size <= mouse_pos[0] <= self.pos[0] + self.size and
                self.pos[1] - self.size <= mouse_pos[1] <= self.pos[1]):
            return True
        else:
            return False
    ...
    def render(self):
        if self.highlighted:
            ...
            pygame.draw.lines(self.window, self.colour_1, True, [[self.pos[0] - self.size, self.pos[1] - self.size / 4],
                                                                 [self.pos[0] + self.size, self.pos[1] - self.size / 4],
                                                                 [self.pos[0],
                                                                  self.pos[1] - self.size - self.size / 4]],
                              int(self.size / 10))
        else:
...
class Down_Arrow(Arrow_Button):
    ...
    def collision(self):
        mouse_pos = pygame.mouse.get_pos()
        if (self.pos[0] - self.size <= mouse_pos[0] <= self.pos[0] + self.size and
                self.pos[1] <= mouse_pos[1] <= self.pos[1] + self.size):
            return True
        else:
            return False
...
class Cross_Button:
    ...
    def collision(self):
        mouse_pos = pygame.mouse.get_pos()
        if (self.pos[0] - self.size <= mouse_pos[0] <= self.pos[0] + self.size and
                self.pos[1] - self.size <= mouse_pos[1] <= self.pos[1] + self.size):
            return True
        else:
            return False
    ...
    def render(self):
        if self.highlighted:
            pygame.draw.rect(self.window, self.colour_2,
            ...
            pygame.draw.rect(self.window, self.colour_1,
                             [self.pos[0] - self.size, self.pos[1] - self.size, self.size * 2, self.size * 2])
...
class Ball_Creator:
    def __init__(self, window, pos, resolution):
        ...
        self.delete = Cross_Button(window, [pos[0] + resolution[1] / 27 * 14, pos[1]], resolution[1] / 36,
                                   const.COL["red"], const.COL["white"])

        self.colour_up = Up_Arrow(window, [pos[0] - resolution[1] / 2.16, pos[1]], resolution[1] / 54,
                                  const.COL["light_grey"], const.COL["grey"])
        self.colour_down = Down_Arrow(window, [pos[0] - resolution[1] / 2.16, pos[1]], resolution[1] / 54,
                                      const.COL["light_grey"], const.COL["grey"])

        self.radius_up = Up_Arrow(window, [pos[0], pos[1]], resolution[1] / 54, const.COL["light_grey"],
                                  const.COL["grey"])
        self.radius_down = Down_Arrow(window, [pos[0], pos[1]], resolution[1] / 54, const.COL["light_grey"],
                                      const.COL["grey"])

        self.mass_up = Up_Arrow(window, [pos[0] + resolution[1] / 54 * 19, pos[1]], resolution[1] / 54,
                                const.COL["light_grey"], const.COL["grey"])
        self.mass_down = Down_Arrow(window, [pos[0] + resolution[1] / 54 * 19, pos[1]], resolution[1] / 54,
                                    const.COL["light_grey"], const.COL["grey"])
    ...
    def render(self, resolution):
        pygame.draw.rect(self.window, const.COL["grey"],
                         [self.pos[0] - resolution[1] / 1.8, self.pos[1] - resolution[1] / 27, resolution[1] / 0.9,
                          resolution[1] / 13.5])
        if self.radius >= resolution[1] / 216 * 7:
            pygame.draw.circle(self.window, self.ball_colours[self.colour],
                               [int(self.pos[0] - resolution[1] / 27 * 14), int(self.pos[1])],
                               int(resolution[1] / 216 * 7))
        else:
            pygame.draw.circle(self.window, self.ball_colours[self.colour],
                               [int(self.pos[0] - resolution[1] / 27 * 14), int(self.pos[1])], self.radius)
        draw_text(self.window, "RADIUS: {}".format(self.radius),
                  [self.pos[0] - resolution[1] / 108 * 37, self.pos[1] + 3], resolution[0] / 30, const.COL["white"],
                  "calibri", "ml")
        draw_text(self.window, "MASS: {}".format(self.mass), [self.pos[0] + resolution[1] / 18, self.pos[1] + 3],
                  resolution[0] / 30, const.COL["white"], "calibri", "ml")
        self.delete.render()
...
class Add_Creator:
    ...
    def collision(self, resolution):
        mouse_pos = pygame.mouse.get_pos()
        if (self.pos[0] - resolution[1] / 1.8 <= mouse_pos[0] <= self.pos[0] + resolution[1] / 1.8 and
                self.pos[1] - resolution[1] / 27 <= mouse_pos[1] <= self.pos[1] + resolution[1] / 27):
            return True
        else:
            return False
    ...
    def render(self, resolution):
        if self.highlighted:
            pygame.draw.rect(self.window, const.COL["light_grey"],
                             [self.pos[0] - resolution[1] / 1.8, self.pos[1] - resolution[1] / 27, resolution[1] / 0.9,
                              resolution[1] / 13.5])
            pygame.draw.rect(self.window, const.COL["grey"],
                             [self.pos[0] - resolution[1] / 1.8, self.pos[1] - resolution[1] / 27, resolution[1] / 0.9,
...
class Scroll_Bar:
    ...
    def render(self):
        pygame.draw.rect(self.window, self.colour,
                         [self.pos[0] - self.max / 50, self.pos[1] - self.max / 2, self.max / 25, self.max],
                         int(self.max / 250))
        pygame.draw.rect(self.window, self.colour,
                         [self.pos[0] - self.max / 50, self.slider_pos + self.max / 25, self.max / 25,
                          self.slider_size])

    def dragging(self):
        if pygame.mouse.get_pressed()[0]:
            mouse_pos = pygame.mouse.get_pos()
            if (self.pos[0] + self.max / 25 >= mouse_pos[0] >= self.pos[0] and
                mouse_pos[1] <= self.slider_pos + self.slider_size + self.max / 25 and mouse_pos[
                    1] >= self.slider_pos + self.max / 25) or self.grabbed:
