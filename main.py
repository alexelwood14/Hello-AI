    ...
    def run(self):
        while True:
            if self.mode == const.MODE.MAIN:
                ...
                self.mode, mouse_used, self.window, resolution = settings(self.window, self.resolution,
                                                                          self.mode, self.buttons, self.mouse_used)
...
def init_objects(window):
    ...
    increment = window.get_size()[1] * 7 / 54
    quit_button = pygame_ui.Single_Button(window, [window.get_size()[1] / 3.6, start + increment * 4],
                                          window.get_size()[0] / 5, window.get_size()[0] / 30, "QUIT",
                                          const.COL["black"], const.COL["white"])
    start_button = pygame_ui.Single_Button(window, [window.get_size()[1] / 3.6, start + increment * 2],
                                           window.get_size()[0] / 5, window.get_size()[0] / 30, "START",
                                           const.COL["black"], const.COL["white"])
    back_button = pygame_ui.Single_Button(window,
                                          [window.get_size()[0] / 2, window.get_size()[1] - window.get_size()[0] / 16],
                                          window.get_size()[0] / 5, window.get_size()[0] / 30, "BACK",
                                          const.COL["black"], const.COL["white"])
    settings_button = pygame_ui.Single_Button(window, [window.get_size()[1] / 3.6, start + increment * 3],
                                              window.get_size()[0] / 5, window.get_size()[0] / 30, "SETTINGS",
                                              const.COL["black"], const.COL["white"])
    apply_button = pygame_ui.Single_Button(window, [window.get_size()[1] / 3.6, start + increment * 3],
                                           window.get_size()[0] / 5, window.get_size()[0] / 30, "APPLY",
                                           const.COL["black"], const.COL["white"])

    # Initiate Arrows Buttons
    res_up = pygame_ui.Up_Arrow(window, [window.get_size()[0] / 2 - window.get_size()[0] / 5.5,
                                         window.get_size()[1] / 3], window.get_size()[1] / 54,
                                const.COL["white"], const.COL["black"])
    res_down = pygame_ui.Down_Arrow(window, [window.get_size()[0] / 2 - window.get_size()[0] / 5.5,
                                             window.get_size()[1] / 3], window.get_size()[1] / 54,
                                    const.COL["white"], const.COL["black"])
    screen_up = pygame_ui.Up_Arrow(window, [window.get_size()[0] / 2 - window.get_size()[0] / 5.5,
                                            window.get_size()[1] / 2], window.get_size()[1] / 54,
                                   const.COL["white"], const.COL["black"])
    screen_down = pygame_ui.Down_Arrow(window, [window.get_size()[0] / 2 - window.get_size()[0] / 5.5,
                                                window.get_size()[1] / 2], window.get_size()[1] / 54,
                                       const.COL["white"], const.COL["black"])
...
def settings(window, resolution, action, buttons, mouse_used):
    ...
    used_buttons["apply"].set_pos([resolution[0] / 5, start + increment * 2])
    used_buttons["back"].set_pos([resolution[0] / 5, start + increment * 3])
    ...
    while action == const.MODE.SETTINGS:
        ...
        pygame_ui.draw_text(window, 'Settings', [resolution[0] / 5, resolution[1] / 10.8], int(resolution[1] / 10.5),
                            const.COL["white"], 'impact', 'c')

        pygame_ui.draw_text(window, '{}x{}'.format(resolutions[res][0], resolutions[res][1]),
                            [resolution[0] / 6, resolution[1] / 3], int(resolution[1] / 15),
                            const.COL["white"], 'calibri', 'c')
        if fullscreen:
            pygame_ui.draw_text(window, 'FULLSCREEN', [resolution[0] / 6, resolution[1] / 2], int(resolution[1] / 15),
                                const.COL["white"], 'calibri', 'c')
        else:
            pygame_ui.draw_text(window, 'WINDOWED', [resolution[0] / 6, resolution[1] / 2], int(resolution[1] / 15),
                                const.COL["white"], 'calibri', 'c')
...
def main_menu(window, action, buttons, mouse_used):
    ...
    used_buttons["settings"].set_pos([window.get_size()[0] / 5, start + increment])
    used_buttons["quit"].set_pos([window.get_size()[0] / 5, start + increment * 2])

    increment_1 = window.get_size()[1] / 10
    ...
    while action == const.MODE.MAIN:
        ...
        pygame_ui.draw_text(window, 'Artificial', [window.get_size()[0] / 5, window.get_size()[1] / 10.8],
                            int(window.get_size()[1] / 10.5), const.COL["white"], 'impact', 'c')
        pygame_ui.draw_text(window, 'Intelligence',
                            [window.get_size()[0] / 5, window.get_size()[1] / 10.8 + increment_1],
                            int(window.get_size()[1] / 10.5), const.COL["white"], 'impact', 'c')
        pygame_ui.draw_text(window, 'Alex Elwood | BETA (0.0.1)',
                            [window.get_size()[0] / 1.007, window.get_size()[1] / 1.02], int(window.get_size()[1] / 56),
                            const.COL["white"], 'calibri', 'mr')
        pygame.display.update()
